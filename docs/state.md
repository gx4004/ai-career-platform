...
| R16 source/user/engine gates built ahead of activation evidence | #189 adds a dark one-to-one source legal/contract/promotion/kill gate. #190 adds an owner/source grant record containing only a closed OAuth mechanism class, fixed submission scope, and timestamp, with no provider credentials, tokens, sessions, provider account identifiers, or arbitrary callback content. #191 adds an internal adapter-neutral engine that resolves fields only from the immutable approved snapshot, checks all four gates, and uses a deterministic snapshot/source key plus structural uniqueness across retries and restarts. Only local fixture adapters exist. No public grant-creation or submission route, source OAuth or real submission adapter, real source, scheduler, production envelope, or outward-act endpoint exists. | Keep the R16 outcome and production activation behind D-100. Do not register/promote a real source without accepted legal review; do not add an OAuth callback or adapter until its source contract and security design are approved; #193 must supply and rehearse the authoritative envelope before activation; preserve exact grant rechecks, packet integrity, and all D-026 prohibitions. |
