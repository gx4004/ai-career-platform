...
| R7 #110 built dark ahead of the R6 baseline (owner-authorized, thesis completion) | `docs/roadmap.md` and this file's Session Handoff list R7 (#110–#115) as blocked on the two-week R6 activation baseline before any candidate ships. Under explicit owner authorization for thesis completion, R7 #110 (first-run entry choice) is now built **dark**: default OFF behind `VITE_R7_ENTRY_CHOICE` (frontend `src/lib/flags/featureFlags.ts`), so the roadmap's "no candidate ships live until R6's baseline exists" gate is preserved — nothing changes for real users. This is a deliberate build-ahead, not a live activation, and does not close the R6 gate. | Do NOT set `VITE_R7_ENTRY_CHOICE=true` in any live environment until the R6 baseline confirms the drop-off it targets (PRD #109). Sibling R7 tickets reuse the same per-candidate `VITE_R7_*` flag seam. |
