...
| R16 source/user gates built ahead of activation evidence | #189 adds a dark one-to-one source legal/contract/promotion/kill gate. #190 adds an owner/source grant record containing only a closed OAuth mechanism class, fixed submission scope, and timestamp, with no provider credentials, tokens, sessions, provider account identifiers, or arbitrary callback content. Grants can be listed and revoked owner-scoped, are included in the machine-readable export/erasure cascade, and stale work must re-check its exact grant id. No public grant-creation route, source OAuth adapter, real source, submission integration, queue, or outward-act endpoint exists. | Keep the R16 outcome and production activation behind D-100. Do not register/promote a real source without accepted legal review; do not add an OAuth callback until its source contract and security design are approved; require the exact grant-id checkpoint before every future outward act; preserve the packet/envelope gates and all D-026 prohibitions. |
